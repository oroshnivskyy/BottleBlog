...
# implement the function make_pw_hash(pw) that returns a hashed password 
# use bcrypt
...
# validates that password is right
def check_pw(password, password_encrypted):
    import bcrypt
    hash = ""
    try:
        hash = bcrypt.hashpw(password, password_encrypted)
    except:
        print sys.exc_info()[0]
    return hash == password_encrypted
...
def validate_login(connection, username, password, user_record):
    ...
    if (check_pw(password,user['password'])==False):
